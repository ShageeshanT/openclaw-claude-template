...
app.get("/setup/api/status", requireSetupAuth, async (_req, res) => {
  ...
  const authGroups = [
    ...
    },
    {
      value: "anthropic",
      label: "Anthropic",
      hint: "API key or Claude Code subscription",
      options: [
        { value: "apiKey", label: "Anthropic API key" },
        {
          value: "claude-cli",
          label: "Claude Code subscription (login via SSH, no API key needed)",
          hint: "uses ~/.claude/ creds — Pro/Max",
        },
      ],
    },
    {
...
app.get("/setup/api/claude-cli/status", requireSetupAuth, async (_req, res) => {
  res.json({ ok: true, ...claudeCliStatus() });
});
...
function buildOnboardArgs(payload) {
  ...
  if (payload.authChoice && payload.authChoice !== "claude-cli") {
...
// Path where the symlinked persistent Claude Code credentials live.
// entrypoint.sh links /home/openclaw/.claude -> /data/.claude so this
// resolves to the volume across redeploys.
const CLAUDE_HOME_DIR = path.join(os.homedir(), ".claude");
const CLAUDE_CREDENTIALS_FILE = path.join(CLAUDE_HOME_DIR, ".credentials.json");

function claudeCliStatus() {
  // We treat the presence of `.credentials.json` as "logged in".
  // The Claude Code CLI writes that file after `claude` finishes the device
  // OAuth flow; if it's missing the user still needs to SSH in and run `claude`.
  let loggedIn = false;
  let credentialsAt = null;
  try {
    if (fs.existsSync(CLAUDE_CREDENTIALS_FILE)) {
      loggedIn = true;
      credentialsAt = CLAUDE_CREDENTIALS_FILE;
    } else if (fs.existsSync(CLAUDE_HOME_DIR)) {
      // Some Claude CLI builds use a different filename — surface the dir
      // so the user can still tell if they ran `claude` at all.
      const entries = fs.readdirSync(CLAUDE_HOME_DIR);
      if (entries.length > 0) {
        loggedIn = true;
        credentialsAt = CLAUDE_HOME_DIR;
      }
    }
  } catch {}
  return { loggedIn, credentialsAt, claudeHome: CLAUDE_HOME_DIR };
}
...
app.post("/setup/api/run", requireSetupAuth, async (req, res) => {
  try {
    ...
    // Pre-flight: refuse to start onboarding in Claude-CLI mode unless the
    // user has actually logged in via `railway ssh` + `claude` first.
    // Otherwise OpenClaw would onboard with no Anthropic credentials and
    // every request would 401 until the user noticed.
    if (payload.authChoice === "claude-cli") {
      const status = claudeCliStatus();
      if (!status.loggedIn) {
        return res.status(400).json({
          ok: false,
          output:
            "Claude Code subscription auth is selected, but no credentials were found at " +
            `${CLAUDE_HOME_DIR}.\n\n` +
            "Log in once via Railway:\n" +
            "  1. railway ssh\n" +
            "  2. claude    (paste the device-flow URL into your browser)\n" +
            "  3. exit\n\n" +
            "Then re-run setup. Credentials persist on the /data volume forever.\n",
        });
      }
    }
    ...
    if (ok) {
      ...
      if (payload.authChoice === "claude-cli") {
        extra += "\n[setup] Registering Claude Code CLI as Anthropic backend...\n";
        const cliLogin = await runCmd(OPENCLAW_NODE, clawArgs([
          "models",
          "auth",
          "login",
          "--provider",
        ...
        extra += `[claude-cli] models auth login exit=${cliLogin.code}\n${cliLogin.output || "(no output)"}\n`;
        if (cliLogin.code !== 0) {
          extra +=
            "\n[claude-cli] WARNING: openclaw could not register the Claude CLI backend.\n" +
            "Verify `claude` is on PATH inside the container and that you've run\n" +
            "`railway ssh` + `claude` to complete the device-flow login.\n";
        }
      }
